...
import { Loader2Icon, TrendingUp, RefreshCw } from "lucide-react";
import {
  Pie,
  PieChart,
  CartesianGrid,
...
export default function Page() {
  ...
  const fetchData = async (selectedFilter = filter) => {
    try {
      ...
      const [
        ...
        fetch(`/api/admin/revenue/total?filter=${selectedFilter}`),
        fetch(`/api/admin/expenses/total?filter=${selectedFilter}`),
        fetch(`/api/admin/profit/total?filter=${selectedFilter}`),
        fetch(`/api/admin/cashflow?filter=${selectedFilter}`),
        fetch(`/api/admin/revenue/category?filter=${selectedFilter}`),
        fetch(`/api/admin/expenses/category?filter=${selectedFilter}`),
        fetch(`/api/admin/profit/margin?filter=${selectedFilter}`)
      ]);
  ...
  const handleFilterChange = (newFilter: 'monthly' | 'yearly' | 'all') => {
  ...
  return (
    <div className="grid flex-1 items-start gap-4">
       <div className="flex gap-2 mb-4">
        {['monthly', 'yearly', 'all'].map((range) => (
          <button
            key={range}
            onClick={() => handleFilterChange(range as 'monthly' | 'yearly' | 'all')}
        ...
        <button
          onClick={() => fetchData()}
          className="ml-auto flex items-center gap-1 px-4 py-2 rounded-md text-sm font-medium bg-muted text-muted-foreground hover:bg-accent"
        >
